...
export default function FestiveController() {
  ...
  const [transitioning] = useState(false); // mantido, mas não utilizado ativamente
    ...
    >
      <svg viewBox="0 0 600 600" className="w-[92vw] max-w-[640px]">
        <defs>
          ...
          {/* Path com id para animar um ponto seguindo o contorno */}
          <path id="treeContour" d={treePath} fill="none" />
        </defs>
        ...
        <motion.polygon
          ...
          style={{ filter: "drop-shadow(0 0 10px rgba(244,208,63,.75))" }}
        />
        ...
        <motion.path
          ...
          style={{ filter: "drop-shadow(0 0 10px rgba(227,184,115,.55))" }}
        />
        ...
        <motion.path
          ...
          style={{ filter: "drop-shadow(0 0 8px rgba(227,184,115,.45))" }}
        />
        ...
        {/* luz “percorrendo” o contorno usando SVG animateMotion (compatível e sem HTML dentro do SVG) */}
        <g>
          <circle
            r="4"
            fill="#7EE1FF"
            style={{ filter: "drop-shadow(0 0 10px rgba(126,225,255,.75))" }}
          >
            <animate
              ...
              fill="freeze"
            />
            <animateMotion dur="2.2s" begin="0.2s" fill="freeze">
              <mpath href="#treeContour" />
            </animateMotion>
          </circle>
        </g>
...
function GarlandEdge({ side = "top" }: { side?: "top" | "bottom" }) {
  ...
  return (
    <div
      ...
      } left-0 right-0 z-[998] flex justify-center gap-4 py-2`}
    >
      {dots.map((_, i) => (
        <motion.div
          key={i}
